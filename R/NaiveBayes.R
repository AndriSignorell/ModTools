...
NaiveBayes.default <- function(x, y, prior = NULL, laplace = 0,
                                usekernel = FALSE, ...) {

    n <- nrow(x)

    data <- as.data.frame(x)
    lev <- sort(as.character(unique(y)))
    vars <- names(data)

    if (!is.factor(y) && !is.character(y) && !is.logical(y))
        stop("y has to be either a factor or character or logical vector")

    if (is.factor(y) && nlevels(y) != length(lev)) {
    ...
    if (is.null(prior)) {
        ...
        if (length(prior) != length(lev))
            stop(paste0("Vector with prior probabilities should have ",
                        length(lev), " entries"))
        prior <- stats::setNames(prior / sum(prior), lev)
    }
    ...
    cl <- match.call()
    cl[[1L]] <- as.name("NaiveBayes")
    res <- list(data = list(x = data, y = y), lev = lev,
                laplace = laplace, tables = tables, prior = prior,
                usekernel = usekernel, N = n, call = cl)
...
# NaiveBayes.formula <- function(formula, data, prior = NULL, laplace = 0,
...
#         stop("y has to be either a factor or character or logical vector")
#
#     res <- NaiveBayes.default(data, y, prior, laplace, usekernel, ...)
#     res$call <- match.call()
#     res
# }
...
NaiveBayes.formula <- function (formula, data, prior = NULL, laplace = 0,
  ...
  m[c("prior", "laplace", "usekernel", "...")] <- NULL
  ...
  if (xint > 0L)
    x <- x[, -xint, drop = FALSE]
  res <- NaiveBayes.default(x, grouping, prior, laplace, usekernel, ...)
  res$terms <- Terms
  cl <- match.call()
  cl[[1L]] <- as.name("NaiveBayes")
  res$call <- cl
...
# function (object, newdata, type = c("vector", "prob", "class",
#                                     "matrix"), na.action = na.pass, ...)
...
predict.NaiveBayes <- function(object, newdata = NULL, type = c("class", "prob"), na.action = na.pass,
                                threshold = 0.001, ...) {
...
plot.NaiveBayes <- function(x, which = NULL, ask = FALSE, legend = TRUE,
